...
import { parseMovieId } from '../shared/validation';

export async function movieCredits(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    ...
    try {
        ...
        context.error("Error fetching movie credits:", error);
        const message = error instanceof Error ? error.message : '';
        const status = message.startsWith('Invalid ') || message.includes('Movie ID') ? 400 : 500;
        return jsonResponse(req, status, {
            error: status === 400 ? message : "Failed to fetch movie credits"
        });
    }
}

app.http('movieCredits', {
    methods: ['GET', 'OPTIONS'],
    authLevel: 'anonymous',
    route: 'movies/{id}/credits',
    handler: movieCredits,
});